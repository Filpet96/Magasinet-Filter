...
class Quiz extends Component {
  ...
  componentDidMount() {
    ...
    fetch(`${endPoint}/quiz/politicians/`)
      .then(response => response.json())
      .catch(error => {
        this.setState({ error });
      })
    ...
    return (
      <div>
        ...
        <div className="quizContainer">
          ...
          <img
            src={this.state.quiz.featured_image}
          ...
          <div className="bill-bull-quiz">
            <h2 className="uppercase bold quiz_header_text">
              {this.state.quiz.quiz_header_text}
            </h2>
            <p className="quiz_sub_header center">
              {this.state.quiz.quiz_sub_header_content}
            </p>
            <Question
          ...
          <ArticlePreviewSlider
            tag="filterbubblan"
            headerText="Mer från filterbubblan"
          />
          {this.state.quizCompleted && (
            <ArticlePreviewSlider
              tag="tunga-granskningar"
              headerText="Mer från magasinet"
              darkMode={true}
            />
          )}
        </div>
